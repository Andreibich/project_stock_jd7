...
public class SpringdataReceiptController {
    ...
    public ResponseEntity<HibernateReceipt> createReceipt(@RequestBody ReceiptCreateRequest request) {
        ...
        receipt.setReceiptUser(springDataUserDao.findBySurname(request.getUserSurname()));
        ...
        receipt.setReceiptDate(new Date(new Timestamp(System.currentTimeMillis()).getTime()));

        HibernateReceipt savedReceipt = springDataReceiptDao.saveAndFlush(receipt);
